...
Chess.Piece.prototype.validMove = function (startPos, endPos) {
   ...
   if (
      this.board.getPiece(endPos) !== null &&
      this.color === this.board.getPiece(endPos).color
   )
      return false;
...
Chess.Queen = function (color, board, position) {
   ...
   this.show =
      this.color === "black"
         ? this.role("black", "Queen")
         : this.role("white", "Queen");
};
...
Chess.King = function (color, board, position) {
   ...
   this.show =
      this.color === "black"
         ? this.role("black", "King")
         : this.role("white", "King");
};
   ...
   for (var i = 0; i < this.board.grid.length; i++) {
      for (var j = 0; j < this.board.grid[i].length; j++) {
         ...
         if (
            ...
            !(piece instanceof Chess.King)
         ) {
            if (Chess.Util._includesSubArray(piece.availableMoves(), position))
               return true;
         }
      }
   }
...
Chess.King.prototype.checkIfCastleMove = function (endPos) {
   ...
   if (
      this.color === "white" &&
      Chess.Util._arrayEquals(endPos, [7, 2]) &&
      castlingDirection[0]
   ) {
      return true;
   } else if (
      this.color === "white" &&
      Chess.Util._arrayEquals(endPos, [7, 6]) &&
      castlingDirection[1]
   ) {
      return true;
   } else if (
      this.color === "black" &&
      Chess.Util._arrayEquals(endPos, [0, 2]) &&
      castlingDirection[0]
   ) {
      return true;
   } else if (
      this.color === "black" &&
      Chess.Util._arrayEquals(endPos, [0, 6]) &&
      castlingDirection[1]
   ) {
      return true;
   }
...
Chess.King.prototype.castlingCollisionCheck = function (direction) {
   for (var i = 0; i < 2; i++) {
      if (this.color === "white" && direction === "left") {
         if (
            this.board.getPiece([7, 3 - i]) !== null ||
            this.inCheck([7, 3 - i])
         )
            return false;
      } else if (this.color === "white" && direction === "right") {
         if (
            this.board.getPiece([7, 5 + i]) !== null ||
            this.inCheck([7, 5 + i])
         )
            return false;
      } else if (this.color === "black" && direction === "right") {
         if (
            this.board.getPiece([0, 5 + i]) !== null ||
            this.inCheck([0, 5 + i])
         )
            return false;
      } else if (this.color === "black" && direction === "left") {
         if (
            this.board.getPiece([0, 3 - i]) !== null ||
            this.inCheck([0, 3 - i])
         )
            return false;
      }
...
// // tambahan
// Chess.Piece.prototype.getPiecesData = async function (color) {
//    let role = color; // values ("white","black")
//    let path = `./js/json/${role}-pieces.json`; // just a path of json file
//    let data = await fetch(path); // get the file content

//    this.piecesData = await data.json(); // convert data as json
// };

// tambahan
Chess.Piece.prototype.role = function (color, role) {
   let img = "";

   if (!color || !role) console.log("fungsi tambahan error");

   if (color === "black") {
      if (role === "Pawn" || role === "pawn") {
         img = `<img src="./images/pieces/bp.png">`;
      } else if (role === "Rook" || role === "rook") {
         img = `<img src="./images/pieces/br.png">`;
      } else if (role === "Knight" || role === "knight") {
         img = `<img src="./images/pieces/bn.png">`;
      } else if (role === "Bishop" || role === "bishop") {
         img = `<img src="./images/pieces/bb.png">`;
      } else if (role === "Queen" || role === "queen") {
         img = `<img src="./images/pieces/bq.png">`;
      } else if (role === "King" || role === "king") {
         img = `<img src="./images/pieces/bk.png">`;
      } else {
   ...
   if (color === "white") {
      if (role === "Pawn" || role === "pawn") {
         img = `<img src="./images/pieces/wp.png">`;
      } else if (role === "Rook" || role === "rook") {
         img = `<img src="./images/pieces/wr.png">`;
      } else if (role === "Knight" || role === "knight") {
         img = `<img src="./images/pieces/wn.png">`;
      } else if (role === "Bishop" || role === "bishop") {
         img = `<img src="./images/pieces/wb.png">`;
      } else if (role === "Queen" || role === "queen") {
         img = `<img src="./images/pieces/wq.png">`;
      } else if (role === "King" || role === "king") {
         img = `<img src="./images/pieces/wk.png">`;
      } else {
         console.log("what is this ?");
      }
   }
...
Chess.Knight = function (color, board, position) {
   ...
   this.show =
      this.color === "black"
         ? this.role("black", "Knight")
         : this.role("white", "Knight");
};
...
Chess.Bishop = function (color, board, position) {
   ...
   this.show =
      this.color === "black"
         ? this.role("black", "Bishop")
         : this.role("white", "Bishop");
...
Chess.Rook = function (color, board, position) {
   ...
   this.show =
      this.color === "black"
         ? this.role("black", "Rook")
         : this.role("white", "Rook");
};
...
Chess.Pawn = function (color, board, position) {
   ...
   this.show =
      this.color === "black"
         ? this.role("black", "Pawn")
         : this.role("white", "Pawn");
};
...
Chess.Pawn.prototype.validMove = function (startPos, endPos) {
   ...
   if (
      ...
      y === b &&
      Math.abs(x - a) === 2 &&
      this.board.getPiece([a - 1, b]) === null
   ) {
      return true;
   } else if (
      this.color === "black" &&
   ...
   if (
      ...
      this.board.grid[x][y].color != this.color
   ) {
      return true;
   } else if (
      this.color === "black" &&
   ...
   var expression =
      this.color === "black" ? x - a === 1 && y === b : x - a === -1 && y === b;
...
Chess.Pawn.prototype.toQueen = function () {
   ...
   this.board.grid[x][y] = new Chess.Queen(
      this.color,
      this.board,
...
Chess.Pawn.prototype.toBishop = function () {
   var x = this.currentPosition[0];
   var y = this.currentPosition[1];
   this.board.grid[x][y] = new Chess.Bishop(
      this.color,
      this.board,
      this.currentPosition
   );
...
Chess.Pawn.prototype.toKnight = function () {
   var x = this.currentPosition[0];
   var y = this.currentPosition[1];
   this.board.grid[x][y] = new Chess.Knight(
      this.color,
      this.board,
      this.currentPosition
   );
...
Chess.Pawn.prototype.toRook = function () {
   var x = this.currentPosition[0];
   var y = this.currentPosition[1];
   this.board.grid[x][y] = new Chess.Rook(
      this.color,
      this.board,
      this.currentPosition
   );
