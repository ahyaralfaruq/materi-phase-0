...
class Chess {
   ...
   start() {
      ...
      this.data.players.forEach((p) => p.startTimer()); // start the player 14.28
   }
   ...
   // switch player to another player
   switchTurn(player) {
      const players = this.data.players;
   ...
   moved(...param) {
      this.data.board.resetSquares(); // reset possible squares id
      this.changeTurn(); // when player move, change turn player
   ...
   async loadMatchHistory(matchHistoryJsonFile) {
      ...
      if (isString) {
         try {
            ...
            // if something not right throw an error
   ...
   async previewMatchHistory(matchHistory, index = 0) {
      ...
      const moveTo = async function (player, piece, square) {
         return new Promise((resolve) => {
            setTimeout(function () {
               if (!piece) resolve(); // if piece is undefined justt return
      ...
      for (let i = 0; i < matchHistory.length; i++) {
         ...
         // if there's an error  into a movementt then throw an error
         if (!move)
            throw new Error(
               `Oops something is wrong, Movement ${matchHistory[i].from} to ${matchHistory[i].to} is not right`
            );
   ...
   isMate() {
      ...
      if (playerTurn.info.isChecked) {
         ...
         // if there's no possible move, and king was dont have move also
         // then checkmate
...
class Piece {
   ...
   // when there's piece inside the target square, eat then
   eat(piece) {
      if (!piece) return;
      ...
      // insert into the target player dropped piece
      piecePlayer.data.dropped.push(piece);

      // remove piece into the target player piece
   ...
   move(square, castling) {
      ...
      // trigger, finished moves
      this.game.moved(old, square);

      // is the move is castling, then castle
   ...
   castling() {
      ...
      const check = function (piece, square, condition) {
         // move only the condition is true
         if (!condition) return;
      ...
      // check each rock
      check(rr, board[y][x - 1], rr && rr.info.name === "Rock");
      check(lr, board[y][x + 1], lr && lr.info.name === "Rock");
   }
   ...
   listener() {
      ...
      const mousedown = function (event) {
         ...
         // if player is reviewing match history
         // return false
         ...
         const setStyle = function () {
            // set the position tot absolute so the image can drag anywhere on the screen
         ...
         // get the piece possibilities, values(moves(enemy),  enemies(array), castling(array))
         // then show circles to all that square
   ...
   // get piece possibilities, move, enemies, castling
   getPossibilities() {
      ...
      // test the square when piece can bee move or there is enemy
      ...
      // yi/xi = y/x read to change
      // yo / xo = what operation, true = addition while, false = subtraction
      // un = until (number), how many squares read to check
         ...
         for (let i = 1; i < un; i++) {
            ...
            if (square) {
               if (sqpiece) {
                  ...
                  const rightRock = board[ny][nx + 1].piece;
                  const leftRock = board[ny][nx - 2].piece;

                  check(rightRock && rightRock.info.name === "Rock");
                  check(leftRock && leftRock.info.name === "Rock");
               }
      ...
      const Pattern = {
         Pawn: function () {
            ...
            for (let i = 1; i < until; i++) {
               if (role === "white") {
                  // if it is white, substract current i value
                  // so it moves from bottom to top
                  if (!testSquare(y - i, x)) break;
               } else {
                  // if it is black, substract current i value
            ...
            if (role === "white") {
               ...
               // (black) check the top left and right square from it's position
               testEnemy(y + 1, x - 1);
               testEnemy(y + 1, x + 1);
            }
         },
         ...
         Queen: function () {
            Pattern.Rock(); // can move like a Rock
...
class Square {
   ...
   // create ul
   create() {
      const squareElement = document.createElement("div"); // row div element
   ...
   listener() {
      ...
      const action = function () {
         ...
         // move the player on the selected square
...
class Player {
   constructor(player) {
      this.info = {
         ...
         isLeave: false, // is player was leave
   ...
   move(piece, square) {
      ...
      const game = this.game; // the game
      const test = game.testMove(piece, square); // test first the move, will return boolean
      const info = square.info; // square informations
      const isQualified = info.isMove || info.isEnemy || info.isCastle; // weather move, enemy or castle
      ...
      // if there is no wrong in move, then move
   ...
   startTimer() {
      ...
      const card = player.data.card; // player card element
      ...
      let currentDuration = parseInt(60 * m) + parseInt(s) ?? 0;
      let duration = m ? currentDuration : game.setTime(game.info.timer);
   ...
   async getPieces() {
      ...
      let path = `./assets/json/${role}-pieces.json`; // just a path of json file
   ...
   async setPieces() {
      ...
      const set = function (setPieceObj) {
         ...
         for (let i = 0; i < length; i++) {
            ...
            const piece = new Piece(obj, player, game); // now piece

            pieces.push(piece); // insert to the array of class Piece
         }
      };

      // loop through all the data then generate some Place base on their length
      // and game rules
...
Game.init(function () {
   this.start();
   Game.loadMatchHistory("./assets/json/match-history.json");
}); // initialize
